...
class arithmeticExpression{
  ...
  public:
    ...
    /* Implementation of destrutor is optional.
    ...
    string getPostfix(TreeNode *root, string& butthole);

    /* Calls the recursive visualizeTree function and generates the .png file using system call. */
    void visualizeTree(const string &);

    /*Evaluates the arithmetic expression, it throws an error if the expression cannot be evaluated.*/
    int evaluate();
 ...
 private:
    ...
    /* Throws an error with the message "expression not valid" if the expression is not a valid integer arithmetic expression. */
    bool isValidIntegerExpression(TreeNode *);

    /* Calls the recursive evaluateIntegerExpression function. */
    int evaluateIntegerExpression(TreeNode *);

    bool detectInt (char ok);

    int performOp(char op, char leftVal, char rightVal);
